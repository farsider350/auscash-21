...
$query_=mysql_query("SELECT * FROM `games` WHERE `bet_amount`!=0 ORDER BY `time` DESC LIMIT $lima,$perPage");
$pocet=mysql_num_rows(mysql_query("SELECT `id` FROM `games` WHERE `bet_amount`!=0"));
...
<table class="vypis_table">
  ...
  while ($row=mysql_fetch_array($query_)) {
    ...
    echo '<td><small>'.$row['winner'].'</small></td>';
    echo '</tr>'."\n";
  }
