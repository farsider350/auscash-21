    ...
    <table style="border: 0; border-collapse: collapse;">
      <tr>
        <td style="padding: 0; vertical-align: middle;">
          ...
          <br><br>
          <b>Free balance:</b><br>
          <big><?php $usersdeps_=mysql_fetch_array(mysql_query("SELECT SUM(`amount`) AS `sum` FROM `deposits`")); $usersdeps_['sum']=(0+(double)$usersdeps_['sum']);  $usersbal_=mysql_fetch_array(mysql_query("SELECT SUM(`balance`) AS `sum` FROM `players`")); $usersbal_['sum']=(0+(double)$usersbal_['sum']); echo n_num($wal_balance-$usersbal_['sum']-$usersdeps_['sum'],true); ?></big> <?php echo $settings['currency_sign']; ?>
        </td>
